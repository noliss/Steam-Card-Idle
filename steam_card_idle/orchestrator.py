...
class Orchestrator:
    ...
    def _note_inventory_drops(self) -> list[CardDrop]:
        ...
        try:
            ...
            self.log(f"Inventory: failed to read ({exc})")
            return []
        if not self.drops.inventory_primed:
            self.drops.seed_inventory(cards)
            self.log(f"Inventory: baseline {len(cards)} trading cards (new ones count as drops)")
            return []
        ...
        for drop in found:
            if drop.card_name:
                ...
                self.log(
                    ...
                    f"(remaining {drop.remaining_after})"
                )
        if found:
            try:
                self.drops.save()
    ...
    def stop(self) -> None:
        ...
        self.emit(phase=Phase.IDLE, message="Stopped")

    def pause(self) -> None:
        self._pause.set()
        self.manager.stop_all()
        self.emit(phase=Phase.PAUSED, message="Paused")
    ...
    def _run_inner(self) -> None:
        if not is_steam_running():
            raise RuntimeError("Steam is not running. Open Steam and sign in.")

        self.emit(phase=Phase.LOADING, message="Signing in…")
        self.session = build_session(self.cfg, allow_offline=True)
        if self.session.offline:
            self.log(
                "Steam Community did not respond — continuing with saved profile"
            )
        self.log(f"Profile: {self.session.profile_url}")

        self.manager.ensure_ready()
        self.emit(phase=Phase.LOADING, message="Reading badges…")
        badges: list[Badge] = []
        if self.session.offline and self._seed_badges:
            badges = list(self._seed_badges)
            self.log(f"Offline start — UI cache ({len(badges)} games)")
        else:
            try:
                ...
                if self._seed_badges:
                    badges = list(self._seed_badges)
                    self.log(
                        f"Badges from Steam unavailable ({exc}) — "
                        f"starting from UI cache ({len(badges)} games)"
                    )
                else:
                    raise
        self.drops = DropTracker()
        self.drops.seed(badges)
        self.emit(badges=badges, message=f"Games with drops: {len(badges)}")
        self.log(f"Games with drops: {len(badges)} (cards: {total_drops(badges)})")
        self.log("Drop tracking: Steam inventory (primary) + badges")
        try:
            inv = fetch_trading_cards(self.session)
            self.drops.seed_inventory(inv)
            self.log(f"Inventory: baseline of {len(inv)} cards")
        except Exception as exc:
            self.log(f"Inventory unavailable at start ({exc}) — falling back to badges")

        if not badges:
            self.emit(phase=Phase.DONE, message="No cards left — idling complete")
    ...
    def _fast_loop(self, badges: list[Badge]) -> None:
        ...
        def sync_starts(queue: list[Badge]) -> None:
            ...
            for app_id in list(active):
                ...
                if app_id not in alive:
                    self.log(f"Worker dead/stale {app_id} — removing from active")
            ...
            for badge in queue:
                ...
                if attempts > max_n * 2 and not active:
                    self.log("Too many idle start failures — stopping attempts.")
                    break
                try:
                    self.manager.start(badge.app_id)
                    ...
                    self.log(f"Failed to idle {badge.app_id}: {exc}")
        ...
        self.emit(
            ...
            badges=queue,
            message=f"Wave: starting up to {cap} games…",
        )
        sync_starts(queue)
        cap = wave_cap(queue)
        self.emit(
        ...
        while not self._stop.is_set():
            ...
            self.emit(
                phase=Phase.FAST_MULTI,
                badges=queue,
                message=f"Wave #{wave + 1}: starting again…",
            )
            sync_starts(queue)
            cap = wave_cap(queue)
            self.emit(
