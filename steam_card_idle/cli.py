...
def _status_table(orch: Orchestrator) -> Table:
    ...
    table.add_row("Фаза", st.phase.value)
    table.add_row("Статус", st.message or "—")
    table.add_row("Карт осталось", str(st.total_remaining))
    table.add_row("Игр в очереди", str(len(st.badges)))
    table.add_row(
        "Сейчас",
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    try:
        if args.login:
            ...
            console.print("Открываю окно входа Steam…")
        ...
        return 1
    cfg.save()
    console.print(f"[green]OK[/green] профиль: {session.profile_url}")
    return 0
...
def cmd_scan(args: argparse.Namespace) -> int:
    ...
    table = Table(title=f"Дропы: {total_drops(badges)} карт / {len(badges)} игр")
    table.add_column("AppID")
    table.add_column("Игра")
    table.add_column("Карт")
    table.add_column("Часы")
    for b in badges:
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    console.print(f"Steam: {steam or 'не найден'}")
    console.print(f"DLL:   {dll or 'не найден'}")
    console.print(f"Steam running: {is_steam_running()}")
    if not is_steam_running():
        console.print("[red]Запусти Steam и залогинься.[/red]")
        return 1
    if not dll:
        console.print("[red]Нет steam_api64.dll в native/[/red]")
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="Steam Card Idle — фарм Steam Trading Cards",
    )
    p.add_argument("--version", action="version", version=__version__)
    sub = p.add_subparsers(dest="cmd")

    setup = sub.add_parser("setup", help="Сохранить / подтянуть cookies")
    setup.add_argument("--login", action="store_true", help="Окно входа Steam (рекомендуется)")
    ...
    scan = sub.add_parser("scan", help="Показать игры с дропами")
    scan.set_defaults(func=cmd_scan)

    run = sub.add_parser("run", help="Запустить фарм")
    run.add_argument("--fast", action="store_true", help="Fast Mode (по умолчанию из config)")
    run.add_argument("--normal", action="store_true", help="По одной игре")
    ...
    gui = sub.add_parser("gui", help="Открыть GUI")
